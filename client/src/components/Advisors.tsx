...
export const Advisors: FC = () => {
  ...
  const fetchHandler = useCallback(
    async (searchValue: string, shouldShowOnline: boolean) => {
      try {
        ...
        const currentLength = data?.advisors?.length || 0
        ...
        const advisors = newData?.data?.advisors || []
  ...
  if (error) return <Typography className={classes.error}>{ERROR_TEXT}</Typography>
